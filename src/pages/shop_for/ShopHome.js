...
const ShopHome = () => {
    ...
    return (
        <div className="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-6 min-h-[700px]">
            <div className="mx-auto mb-8 max-w-screen-sm lg:mb-16">
                <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-gray-900">Choose one group</h2>
                <select
                    id="selected-group"
                    onChange={handleSelectChange}
                    value="Your groups right here"
                    className="block w-full px-4 py-3 text-base text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                >
                    <option value="Your groups right here" disabled>
                ...
                <p className="font-light text-gray-500 sm:text-xl dark:text-gray-400 mt-2">
            ...
            <div className="grid gap-8 lg:gap-16 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {groupMembers.length !== 0 ? (
                    groupMembers.map((group, key) => (
                        <div key={key + Math.random()} className="text-center text-gray-500 dark:text-gray-400">
                            <img
                                className="mx-auto mb-4 w-36 h-36 rounded-full hover:scale-105 transition duration-300 cursor-pointer"
                                onClick={() => {
                    ...
                    <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-gray-900">No group selected.</h2>
                )}
            </div>
        </div>
    );
